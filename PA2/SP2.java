		...
		try {
			...
			while (!connectionSocket.isClosed()) {
				...
				// If the packet is for transferring a chunk of the file
					...
					toClient.flush();
				} else if (packetType == 3) { //get session key from client
					int encryptedSessionKeySize = fromClient.readInt();
					System.out.println("Getting session key from client...");
					encryptedSessionKey = new byte[encryptedSessionKeySize];
					fromClient.readFully(encryptedSessionKey, 0, encryptedSessionKeySize);

					Cipher decipher = Cipher.getInstance("RSA/ECB/PKCS1Padding");
					decipher.init(Cipher.DECRYPT_MODE, privateKey);
					System.out.println("Decrypting session key with private key...");
					byte[] decryptedSessionKeyByte = decipher.doFinal(encryptedSessionKey);
					System.out.println("Decrypted!");
					sessionKey = new SecretKeySpec(decryptedSessionKeyByte, 0, decryptedSessionKeyByte.length, "AES");
