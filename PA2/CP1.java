...
public class CP1 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			byte[] decryptedNonce = decipher.doFinal(encryptedNonce);

			if (Arrays.equals(nonce, decryptedNonce)) {
				System.out.println("Authenticated.");
			} else {
				toServer.writeInt(4); //tell server to close all connections on their side
			...
			for (int i = 0; i < args.length; i++) {
				...
				for (boolean fileEnded = false; !fileEnded;) {
					...
					Cipher encypt = Cipher.getInstance("RSA/ECB/PKCS1Padding");
					encypt.init(Cipher.ENCRYPT_MODE, publicKey);
					byte[] encryptedFile = encypt.doFinal(fromFileBuffer);
					int encryptNumBytes = encryptedFile.length;
